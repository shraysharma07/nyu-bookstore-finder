-- RDS-compatible version (removes CREATE DATABASE and \c commands)
-- Use this when connecting via DATABASE_URL connection string
...
CREATE TABLE IF NOT EXISTS bookstore_distances (
    id SERIAL PRIMARY KEY,
    bookstore_id INT REFERENCES bookstores(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS student_searches (
    id SERIAL PRIMARY KEY,
...
-- Admins table (for librarian authentication)
...
-- Insert dorms (skip if already exists)
INSERT INTO dorms (name, address) 
SELECT * FROM (VALUES 
    ('Chamberi', 'Chamberi, Madrid, Spain'),
    ('Moncloa', 'Moncloa, Madrid, Spain'),
    ('Malasaña', 'Malasaña, Madrid, Spain')
) AS v(name, address)
WHERE NOT EXISTS (SELECT 1 FROM dorms WHERE name = v.name);
...
INSERT INTO bookstores (name, username, password_hash, phone, address) 
SELECT * FROM (VALUES 
...
) AS v(name, username, password_hash, phone, address)
WHERE NOT EXISTS (SELECT 1 FROM bookstores WHERE username = v.username);

-- Insert courses (skip if already exists)
INSERT INTO courses (code, name, subject_id, professor) 
SELECT * FROM (VALUES 
...
) AS v(code, name, subject_id, professor)
WHERE NOT EXISTS (SELECT 1 FROM courses WHERE code = v.code AND professor = v.professor);

-- Insert books (skip if already exists)
INSERT INTO books (title, author, isbn, subject_id, is_required) 
SELECT * FROM (VALUES 
...
) AS v(title, author, isbn, subject_id, is_required)
WHERE NOT EXISTS (SELECT 1 FROM books WHERE title = v.title AND author = v.author);

-- Link courses to books (skip if already exists)
...
) AS v(course_id, book_id, is_required)
WHERE NOT EXISTS (SELECT 1 FROM course_books WHERE course_id = v.course_id AND book_id = v.book_id);

-- Add inventory for bookstores (skip if already exists)
INSERT INTO inventory (bookstore_id, book_id, price, quantity, availability_status) 
SELECT * FROM (VALUES 
...
) AS v(bookstore_id, book_id, price, quantity, availability_status)
WHERE NOT EXISTS (SELECT 1 FROM inventory WHERE bookstore_id = v.bookstore_id AND book_id = v.book_id);

-- Add bookstore distances from dorms (skip if already exists)
INSERT INTO bookstore_distances (bookstore_id, dorm_id, distance_minutes, walking_distance) 
SELECT * FROM (VALUES 
...
) AS v(bookstore_id, dorm_id, distance_minutes, walking_distance)
WHERE NOT EXISTS (SELECT 1 FROM bookstore_distances WHERE bookstore_id = v.bookstore_id AND dorm_id = v.dorm_id);
...
CREATE INDEX IF NOT EXISTS idx_student_searches_timestamp ON student_searches(search_timestamp);
...
-- Add triggers to update updated_at automatically (drop and recreate to avoid errors)
DROP TRIGGER IF EXISTS update_bookstores_updated_at ON bookstores;
